...
sudo systemctl restart ${bin} && sudo journalctl -u ${bin} -f`}
						...
						{updHeight == 0 || indexOfMv === -1 ? (
							...
							<>
								...
								<CodeSnippet
									theme={theme}
									code={`${beforeMv}tmux new -s ${name}-upgrade "sudo bash <(curl -s https://raw.githubusercontent.com/itrocket-team/testnet_guides/main/utils/autoupgrade/upgrade.sh) -u "${updHeight}" -b ${bin} -n "${path}" -p ${name} -r $rpc_port"`}
								/>
							</>
						)}
					</>
