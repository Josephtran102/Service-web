...
sudo systemctl restart ${bin} && sudo journalctl -u ${bin} -f`}
						...
						{updHeight == 0 || indexOfMv === -1 ? (
							...
							<>
								...
								<CodeSnippet
									theme={theme}
									code={`${beforeMv}tmux new -s ${name}-upgrade "sudo bash -c 'curl -s https://raw.githubusercontent.com/itrocket-team/testnet_guides/main/utils/autoupgrade/upgrade.sh | bash -s -- -u "\${updHeight}\" -b ${bin} -n \"${path}\" -p ${name} -r \"20657\"`"}
								/>
							</>
						)}
					</>
